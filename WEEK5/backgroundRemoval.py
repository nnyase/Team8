...
# Method 4
def get_binary_mask4(myimage, shape):
    ...
    if not shape:
        foreground = cv2.morphologyEx(foreground, cv2.MORPH_OPEN, np.ones([1,int(myimage.shape[1]/8)]))
        foreground = cv2.morphologyEx(foreground, cv2.MORPH_OPEN, np.ones([int(myimage.shape[0]/8),1]))
...
def generateBackgroundMasks(imagesPath, masksPath, method, shape = False):
    ...
    for file in files:
            ...
            if file[-4:] == ".jpg":
                ...
                if method == "method1":
                    ...
                    binary_mask =  get_binary_mask4(image,shape)
                elif method == "method5":
